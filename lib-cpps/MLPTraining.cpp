...
int MLPTrainer::batchTraining(int maxBatches)
{
	return(this->batchTrainingWithCheckPointing(maxBatches, 0, NULL)); 
}; 


int MLPTrainer::batchTrainingWithCheckPointing(int maxBatches, int startBatch, bool doChkPointing)
{	
	if ( !this->initialized ) {
	...
	while (  this->dataProviderp->batchAvailable() && (maxBatches == 0 || batches < maxBatches) ) {
			...
			cout << "Error Value for batch: " << batches << ": " << costval << endl; 
