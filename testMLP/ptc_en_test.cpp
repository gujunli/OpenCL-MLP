...
#define PTC_EN_DB_PATH "../../ptc_dataset/English-ptc/"

void ptc_en_training();
void ptc_en_training2();
void ptc_en_training3();     // training with checkpointing support
void ptc_en_batch_testing();
void ptc_en_single_testing();
void ptc_en_predicting();
...
void ptc_en_training()
{
	...
	*/
	...
	//dataProviderp = new MLPptc_enDataProvider(ptc_en_PATH, MLP_DATAMODE_TRAIN, minibatch, shuffleBatches);
...
void ptc_en_training2()
{
	...
	dataProviderp = new MLPPtcDataProvider(PTC_EN_DB_PATH, MLP_DATAMODE_TRAIN, minibatch, shuffleBatches);
	//dataProviderp = new MLPptc_enDataProvider(ptc_en_PATH3, MLP_DATAMODE_TRAIN, minibatch, shuffleBatches);
	dataProviderp->setupDataProvider();                            // set up the data provider
	totalbatches = dataProviderp->getTotalBatches();
...
void ptc_en_batch_testing()
{
	...
	dataProviderp =	new MLPPtcDataProvider(PTC_EN_DB_PATH, MLP_DATAMODE_TEST, minibatch, shuffleBatches);
	dataProviderp->setupDataProvider();                              // set up the data provider
...
static bool ptc_en_output_matching(float *frameOutput, float *frameLabel, int len)
{
	float element;
...
void ptc_en_single_testing()
{
	...
	while ( dataProviderp->batchAvailable() && (batches < totalbatches) ) {
			...
			for (int i=0; i< testerp->getBatchSize(); i++) {
				 if ( testerp->singleTesting(&inputVectors[i*inVecLen], &labelVectors[i*outVecLen], ptc_en_output_matching) )
                       succNum++;
				 frames++;
			};
...
void ptc_en_predicting()
{
	...
	netProviderp = new MLPNetProvider("./", MLP_NP_NNET_DATA_NEW);
	dataProviderp =	new MLPPtcDataProvider(PTC_EN_DB_PATH, MLP_DATAMODE_PREDICT, minibatch, shuffleBatches);
	dataProviderp->setupDataProvider();                              // set up the data provider
