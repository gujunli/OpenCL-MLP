...
#define PTC_LOWERCASE_DB_PATH "../../ptc_dataset/Lowercase-ptc/"

void ptc_lowercase_training();
void ptc_lowercase_training2();
void ptc_lowercase_training3();     // training with checkpointing support
void ptc_lowercase_batch_testing();
void ptc_lowercase_single_testing();
void ptc_lowercase_predicting();

static bool use_stats = false; 

// do ptc_lowercase_training with randomly initialized weights
void ptc_lowercase_training()
{
	...
	dataProviderp = new DNNPtcDataProvider(PTC_LOWERCASE_DB_PATH,  use_stats, DNN_DATAMODE_SP_TRAIN, minibatch, shuffleBatches);
...
// doing ptc_lowercase_training with checkpointing support
void ptc_lowercase_training3()
{
	struct dnn_tv startv, endv;
	...
	if ( cpManager.cpAvailable() ) {
         ...
         dataProviderp = new DNNPtcDataProvider(PTC_LOWERCASE_DB_PATH,  true, DNN_DATAMODE_SP_TRAIN, minibatch, shuffleBatches);
...
void ptc_lowercase_batch_testing()
{
	...
	dataProviderp =	new DNNPtcDataProvider(PTC_LOWERCASE_DB_PATH,  use_stats, DNN_DATAMODE_TEST, minibatch, shuffleBatches);
	dataProviderp->setupDataProvider();                              // set up the data provider
...
static bool ptc_lowercase_output_matching(float *frameOutput, float *frameLabel, int len)
{
	float element;
...
void ptc_lowercase_single_testing()
{
	...
	configProviderp = new MLPConfigProvider("./", MLP_CP_NNET_DATA_NEW);
	dataProviderp =	new DNNPtcDataProvider(PTC_LOWERCASE_DB_PATH,  true, DNN_DATAMODE_TEST, minibatch, shuffleBatches);
	dataProviderp->setupDataProvider();                              // set up the data provider
	...
	while ( dataProviderp->batchAvailable() && (batches < totalbatches) ) {
			...
			for (int i=0; i< testerp->getBatchSize(); i++) {
				 if ( testerp->singleTesting(&inputVectors[i*inVecLen], &labelVectors[i*outVecLen], ptc_lowercase_output_matching) )
                       succNum++;
				 frames++;
			};
...
void ptc_lowercase_predicting()
{
	...
	configProviderp = new MLPConfigProvider("./", MLP_CP_NNET_DATA_NEW);
	dataProviderp =	new DNNPtcDataProvider(PTC_LOWERCASE_DB_PATH,  true, DNN_DATAMODE_PREDICT, minibatch, shuffleBatches);
	dataProviderp->setupDataProvider();                              // set up the data provider
