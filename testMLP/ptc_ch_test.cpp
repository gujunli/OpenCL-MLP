...
void ptc_ch_training()
{
	...
	*/
	...
	//dataProviderp = new MLPptc_chDataProvider(PTC_CH_PATH, DNN_DATAMODE_TRAIN, minibatch, shuffleBatches);
	dataProviderp = new DNNPtcDataProvider(PTC_CH_DB_PATH, DNN_DATAMODE_TRAIN, minibatch, shuffleBatches);
...
void ptc_ch_training2()
{
	...
	dataProviderp = new DNNPtcDataProvider(PTC_CH_DB_PATH, DNN_DATAMODE_TRAIN, minibatch, shuffleBatches);
	//dataProviderp = new MLPptc_chDataProvider(ptc_ch_PATH3, DNN_DATAMODE_TRAIN, minibatch, shuffleBatches);
	dataProviderp->setupDataProvider();                            // set up the data provider
	totalbatches = dataProviderp->getTotalBatches();
...
void ptc_ch_training3()
{
	...
	if ( cpManager.cpAvailable() ) {
	     ...
	     //dataProviderp = new DNNPtcDataProvider(ptc_ch_PATH, DNN_DATAMODE_TRAIN, minibatch, shuffleBatches);
	     dataProviderp = new DNNPtcDataProvider(PTC_CH_DB_PATH, DNN_DATAMODE_TRAIN, minibatch, shuffleBatches);
...
void ptc_ch_batch_testing()
{
	...
	dataProviderp =	new DNNPtcDataProvider(PTC_CH_DB_PATH, DNN_DATAMODE_TEST, minibatch, shuffleBatches);
	dataProviderp->setupDataProvider();                              // set up the data provider
...
void ptc_ch_predicting()
{
	...
	netProviderp = new MLPNetProvider("./", MLP_NP_NNET_DATA_NEW);
	dataProviderp =	new DNNPtcDataProvider(PTC_CH_DB_PATH, DNN_DATAMODE_PREDICT, minibatch, shuffleBatches);
	dataProviderp->setupDataProvider();                              // set up the data provider
