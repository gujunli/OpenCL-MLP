...
void getCurrentTime(struct dnn_tv *tv)
{
     FILETIME ft;
...
long diff_msec(struct dnn_tv *stv, struct dnn_tv *etv)
{
...
long diff_usec(struct dnn_tv *stv, struct dnn_tv *etv)
{
    if ( (stv != NULL) && (etv != NULL) ) {
...
void dnn_log(const char *header, const char *content)
{
	if ( ! myDNNLog.is_open() )
		   myDNNLog.open("dnn_log.txt",ios_base::out|ios_base::trunc);

    myDNNLog << header << ":" << content << endl;
}

void dnn_log_retval(const char *header, int retVal)
{
	if ( ! myDNNLog.is_open() )
		   myDNNLog.open("dnn_log.txt",ios_base::out|ios_base::trunc);

    myDNNLog << header << ":" << retVal << endl;
};
